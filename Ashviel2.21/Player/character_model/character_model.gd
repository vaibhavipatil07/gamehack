...
var next_weapon_to_load: WeaponModel = null
var hip_rotation_offset: float = -55.0

func _ready() -> void:
	hip_ik.start()
	right_hand_ik.start()
	left_hand_ik.start()
	hip_ik.influence = 0.0
...
func on_combat_status_changed(status: String) -> void:
	match status:
		"non_combat":
			...
			set_ik_influence(hip_ik, 0.0, 0.1)
			set_ik_influence(left_hand_ik, 0.0, 0.1)
			set_ik_influence(right_hand_ik, 0.0, 0.1)
		"combat":
			current_combat_status = CombatStatus.COMBAT
			rotate_model(Vector2.UP, current_mouse_rotation)
			set_ik_influence(hip_ik, 1.0, 0.1)

	animation_tree["parameters/combat_transition/transition_request"] = status

func set_ik_influence(ik: SkeletonIK3D, influence: float, duration: float) -> void:
	var tween: Tween = get_tree().create_tween()
	tween.tween_property(ik, "influence", influence, duration)
...
func _on_camera_camera_rotated(rotation: Vector2) -> void:
	current_mouse_rotation = rotation
	rotate_hip()
	match current_combat_status:
		CombatStatus.NONCOMBAT:
			transform.basis = Basis()
			rotate_object_local(Vector3(0.0, 1.0, 0.0), current_mouse_rotation.x)

func rotate_hip() -> void:
	hip_target.transform.basis = Basis()
	hip_target.rotate_object_local(Vector3(1.0, 0.0, 0.0), current_mouse_rotation.y)
	hip_target.rotate_object_local(Vector3(0.0, 1.0, 0.0), deg_to_rad(hip_rotation_offset))

func rotate_model(angle: Vector2 = Vector2.ZERO, rotation: Vector2 = Vector2.ZERO) -> void:
	var new_angle: float = atan2(angle.x, angle.y) - rotation.x
	armature.transform.basis = Basis()
	armature.rotate_object_local(Vector3(0.0, 1.0, 0.0), new_angle)

func _on_weapon_manager_weapon_changed(weapon: Weapon, model: WeaponModel) -> void:
	set_ik_influence(left_hand_ik, 0.0, 0.1)
	set_ik_influence(right_hand_ik, 0.0, 0.1)
	load_new_weapon(weapon, model)

func load_new_weapon(weapon: Weapon, model: WeaponModel) -> void:
	...
	animation_tree.tree_root.get_node("weapon_change_animation").set_animation(weapon.weapon_change_animation.resource_name)
...
func activate_hand_ik() -> void:
	if current_combat_status == CombatStatus.COMBAT:
		set_ik_influence(right_hand_ik, 0.5, 0.1)
		set_ik_influence(left_hand_ik, 1.0, 0.1)
...
func _on_weapon_manager_weapon_manager_started(status: String, weapon: Weapon, model: WeaponModel) -> void:
	on_combat_status_changed(status)
	load_new_weapon(weapon, model)
